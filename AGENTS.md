...
- The mobile menu's expand/collapse is pure CSS via the `.mobile-menu` class (`global.css`): `grid-template-rows: 0fr` → `.is-open { grid-template-rows: 1fr }`, toggled by a small script that also flips `aria-expanded` and swaps the hamburger/close icons. Gotcha: any border on the *collapsing* element itself still renders at 1px even when its content is squeezed to ~0 height — put dividers (like the panel's top border) on the inner content that gets `overflow: hidden`-clipped, not on the outer `.mobile-menu` container, or you get a permanent stray line when the menu is closed.
